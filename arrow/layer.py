...
from arrow.tuners_utils import BaseTunerLayer, check_adapters_to_merge
...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        # # compute weight for self.fv
        # # lora_A: r * hidden_dim
        # # lora_B: hidden_dim * r
        # lora_BA = self.lora_B[adapter_name].weight @ self.lora_A[adapter_name].weight
        # lora_BA = lora_BA.to(device='cpu', dtype=torch.float32)
        # _, _, v = torch.svd_lowrank(lora_BA)
        # v.to(device='cpu', dtype=torch.bfloat16)
        # self.fv[adapter_name] = v[:, 0]
        # # to device
        # self.fv[adapter_name] = self.fv[adapter_name].to(self.lora_A[adapter_name].weight.device)
    ...
    def init_svd(self, adapter_name):
        # compute weight for self.fv
        # lora_A: r * hidden_dim
        # lora_B: hidden_dim * r
        lora_BA = self.lora_B[adapter_name].weight @ self.lora_A[adapter_name].weight
        lora_BA = lora_BA.to(dtype=torch.float32)
        _, _, v = torch.svd_lowrank(lora_BA)
...
class Linear(nn.Module, LoraLayer):
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            x = x.view(-1, hidden_dim) # (batch_size * sequence_length, hidden_dim)
            torch_result_dtype = result.dtype

            # concat all fv, size = (hidden_dim, num_adapters) 
            adapters = list(self.fv.keys())
            fv = torch.stack([self.fv[adapter] for adapter in adapters], dim=1).to(dtype=x.dtype)
            moe_weights = torch.abs(x @ fv) # (batch_size * sequence_length, num_adapters)
            moe_weights, selected_loras = torch.topk(moe_weights, 2, dim=-1)
            moe_weights = torch.softmax(moe_weights, dim=-1) # (batch_size * sequence_length, 2)

            out_features = self.lora_B[self.one_key].out_features
            final_x = torch.zeros(
                (batch_size * sequence_length, out_features), dtype=x.dtype, device=x.device
            )
            lora_mask = torch.nn.functional.one_hot(selected_loras, num_classes=(len(adapters))).permute(2, 1, 0)

            # Loop over all available experts in the model and perform the computation on each expert
            for lora_idx in range(len(adapters)):
                i_lora_A_weight = self.lora_A[adapters[lora_idx]].weight
                i_lora_B_weight = self.lora_B[adapters[lora_idx]].weight
                lora_dropout = self.lora_dropout[adapters[lora_idx]]
                scaling = self.scaling[adapters[lora_idx]]  

                idx, top_x = torch.where(lora_mask[lora_idx])
                current_x = x[None, top_x].reshape(-1, hidden_dim)
                current_xA_T = F.linear(lora_dropout(current_x), i_lora_A_weight, None)
                current_xA_TB_T = F.linear(current_xA_T, i_lora_B_weight, None)

                ith_lora_result = current_xA_TB_T * scaling
                ith_lora_result = ith_lora_result * moe_weights[top_x, idx, None]
